...
| SEC-15 | **One renderer, on the server, sanitising as the last step before the text leaves the process.** Clients never render markdown themselves — the API returns the source and the rendered HTML | The single renderer is the decision; the server is where it has to live. Email (NTF-02, NTF-12), exported reports and API clients all render this text, so a browser-side renderer means a second implementation in Go — and two sanitisers drift into a vulnerability rather than into a cosmetic difference. Mentions, attachment references and vulnerability autolinks also need data the browser does not hold, so the alternative returns the markdown plus a table of everything already resolved. **Accepted cost:** the interface injects an HTML string, which bypasses the framework's own escaping, so the content security policy is the second line. Reversible — SEC-11 stores the source, so the render target can change later |
