...
| ING-02 | **The build's judgement about its own carried patches is never refuted.** We apply its suppressions; we do not re-decide them | Unchanged in intent, inverted in mechanism. This was written when a producer sent us results already filtered by its suppressions. Under ING-20 we run the scan ourselves and apply those same suppressions, which reaches the same place by a route that does not depend on every producer running a scanner |
