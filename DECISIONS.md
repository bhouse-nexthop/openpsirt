...
| ING-28 | **An upload is a set of documents, not a file.** The component inventory and the vulnerability report arrive as separate documents and are joined on the package identifier | What the reference producer actually emits. An earlier reading — "SBOMs augmented with vulnerability data" — described one file, and no producer examined so far works that way. The scan is deliberately kept separate from the build there, because a build is reproducible and a vulnerability report is not |
| ING-29 | **Suppression statements are ingested with them** | Without them a suppressed finding simply stops arriving, with the component version and the pedigree both unchanged — indistinguishable from a scanner fault, and landing in the unexplained bucket that STA-13 says should be empty. This does not re-decide anything: ING-02 still stands, and the statements are read to *explain* a disappearance, not to argue with it |
