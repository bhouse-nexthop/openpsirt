...
| UIX-27 | **Fenced code blocks are syntax-highlighted, on the server with the rest of the render** | The reason to have code blocks here at all is that people paste patch hunks: under STA-16 and TRI-22 a carried patch is the primary explanation path for our largest producer, so "here is the diff that fixes it" is routine triage. Highlighting on the server is what puts that diff into an exported report and an email still highlighted. It also keeps grammars out of the interface bundle, which UIX-18 cares about |
| UIX-28 | **Highlighting emits classes, never inline styles** | The theme then lives in a stylesheet, so light and dark swap on the client. Inline styles would freeze one palette into the rendered output |
...
| SEC-18 | **Syntax highlighting runs inside the render, before the sanitiser, never after.** The language tag is treated as input and allowlisted; an unknown language renders as a plain code block rather than an error | Order matters: the sanitiser has to permit `<span class>` inside `pre > code` under a class prefix, and it can only do that if it sees the highlighter's output. Reversing the two would mean sanitising, then adding markup nothing checked. The language tag is the small hole — it is attacker-chosen text that ends up in a class attribute. Highlighting also counts against SEC-17's time budget, because highlighters are where unbounded work on hostile input historically lives |
...
| Markdown renderer, sanitiser and highlighter | A CommonMark parser with GitHub extensions and raw HTML off, the sanitiser that runs after it, and a syntax highlighter between them (SEC-11 to SEC-18). The requirements are settled; the three are not. A lexer-based highlighter is preferred over a regex-based one — the well-known highlighters have a history of catastrophic backtracking, and this one runs on text a third party wrote. All three must be permissively licensed under SCP-06 |
