...
| API-19 | **Resolved references travel with the source** — what each mention, attachment reference and autolink resolved to | The half a client cannot do for itself, because it needs data and authorisation checks it does not hold. With those in hand, a client rendering markdown for its own interface gets the same links ours does |
...
| ING-28 | **An upload is an inventory and the build's suppressions.** A producer-supplied vulnerability report is accepted as a secondary path | The primary shape follows from ING-20 — the producer describes what it built and what it has already ruled out, and we do the matching. The secondary path stays because some organisations have scanning they trust or constraints we cannot see |
...
| RNK-04 | **The scanner supplies a severity word, not a score.** Numeric scores come from the feeds in ING-10 | Observed rather than assumed: the reference producer's report rates each issue with a single word and an explicit "other" method, carrying no vector. RNK-01 normalises scores at ingest, and this says where they come from — without the feeds there is nothing to normalise |
...
| SEC-15 | **The policy runs once, on the server, at submission — before the text is stored.** What is permitted, which links survive, and what each reference resolves to. Output formatting is not part of it | Policy is the half that cannot be duplicated, because it is the security control and it needs data and authorisation checks no client holds. Validating before storage means stored text is already known-good rather than known-good-when-rendered. Presentation is then a much smaller claim: markdown goes to every consumer (API-17), and HTML rendering serves our own interface and an email's HTML part. **The sanitiser still runs on render**, because rules change and text stored under the old ones does not re-validate itself. **Accepted cost:** the web interface injects an HTML string, so the content security policy is the second line |
...
| SEC-18 | **The fenced-block language tag is user input.** Allowlisted before it reaches a class attribute; an unknown language renders as a plain code block rather than an error | ```` ```<script> ```` is attacker-chosen text landing in markup. Small, and the only highlighting-related hole left once UIX-27 puts the highlighter after the sanitiser instead of before it — running over already-sanitised DOM, its output never passes through the sanitiser at all |
...
| Markdown parser and sanitiser | A CommonMark parser with GitHub extensions and raw HTML off, plus the sanitiser that runs after it (SEC-11 to SEC-18). The requirements are settled; the pair is not. Both must be permissively licensed under SCP-06 |
...
| Paths settled as not materialised | Held open until a real SBOM could be measured. It was, and walking the edges answers the question in milliseconds on every engine (MDL-18) |
...
| Scanning moved inside the deployment (2026-08-28) | Recorded first as producers sending vulnerability data, with the deployment re-scanning only retained releases. Generalised once it was clear every piece of machinery for it was already decided — the scanner, its database, the offline import, and applying the build's suppressions. The argument that settled it was comparability: a producer-run scanner measures each product by whatever version its pipeline installed (ING-02, ING-20, ING-22, ING-28, ING-29) |
