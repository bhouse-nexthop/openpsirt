...
// Declaring is what the catalogue endpoints need.
//
...
type Declaring struct{ Store func() *catalogue.Store }
...
func registerCatalogue(api huma.API, d Declaring) {
	huma.Register(api, huma.Operation{
		...
		Tags: []string{"Catalogue"}, DefaultStatus: http.StatusCreated,
	...
	huma.Register(api, huma.Operation{
		...
		stream, created, err := store.EnsureStream(ctx, product.ID, in.Body.Name,
			catalogue.Kind(in.Body.Kind), parentID)
		if err != nil {
			return nil, declineDeclaration(err)
		}
	...
	huma.Register(api, huma.Operation{
		...
		Summary: "List a product's branches and tags", Tags: []string{"Catalogue"},
	}, func(ctx context.Context, in *struct {
		Product string `path:"product"`
	...
	huma.Register(api, huma.Operation{
		...
		Path:    "/v1/products/{product}/streams/{stream}/variants",
		Summary: "List the ways a stream is built", Tags: []string{"Catalogue"},
	}, func(ctx context.Context, in *struct {
		Product string `path:"product"`
		Stream  string `path:"stream"`
...
func findStream(ctx context.Context, store *catalogue.Store, product, stream string) (*catalogue.Stream, error) {
...
func declineDeclaration(err error) error {
	switch {
	case errors.Is(err, catalogue.ErrDiffers):
...
// storeFor gives the handlers a catalogue, or says plainly that this process
// has none.
func storeFor(d Declaring) (*catalogue.Store, error) {
	if d.Store == nil {
