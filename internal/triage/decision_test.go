...
func TestSendingAClaimBackTakesItOutOfTheQueueUntilItIsAnswered(t *testing.T) {
	...
	each(t, func(t *testing.T, f *fixture) {
		...
		if _, _, err := f.store.SendBack(ctx, f.reviewer, claimed.ID, "   "); err == nil {
			t.Error("a claim was sent back with no reason")
		}
		if _, _, err := f.store.SendBack(ctx, f.reviewer, claimed.ID,
...
func TestAnsweringWhatWasAskedPutsItBackInTheQueue(t *testing.T) {
	...
	each(t, func(t *testing.T, f *fixture) {
		ctx := t.Context()
		claimed := f.claims(t, f.at())
		if _, _, err := f.store.SendBack(ctx, f.reviewer, claimed.ID, "Say how you checked."); err != nil {
			t.Fatal(err)
		}
...
func TestNobodySendsTheirOwnWordsBack(t *testing.T) {
	...
	each(t, func(t *testing.T, f *fixture) {
		claimed := f.claims(t, f.at())
		if _, _, err := f.store.SendBack(t.Context(), f.triager, claimed.ID,
